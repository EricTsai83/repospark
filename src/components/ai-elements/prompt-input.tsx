...
/* eslint-disable react-refresh/only-export-components, react-hooks/refs, react-hooks/set-state-in-effect, @typescript-eslint/no-unused-vars -- shadcn-generated; provider / attachment / speech-recognition wiring follows upstream ai-elements ergonomics */
  ...
  return (
    <PromptInputHoverCard>
      <HoverCardTrigger asChild>
        <div
          className={cn(
            "group relative flex h-8 cursor-pointer select-none items-center gap-1.5 border border-border px-1.5 font-medium text-sm transition-all hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50",
            className,
          )}
          key={data.id}
          {...props}
...
export const PromptInput = ({
  ...
  useEffect(
    ...
    // Cleanup only fires on unmount; `filesRef` is always current.
    [usingProvider],
  );
